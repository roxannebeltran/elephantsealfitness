...
setwd("~/Publications/In Prep/ROXANNE_LRS Phenotype MS/LRS Manuscript Workflow")
D = read.csv('BeltranHernandez2022Data.csv') #load main spreadsheet
...
D_dup = D[duplicated(D$FieldID) |
        duplicated(D$FieldID, fromLast = TRUE), c("FieldID", "Mass", "Preg", "AgeWhenTracked")]
D_dup[order(D_dup$FieldID, D_dup$Mass),]
modelmass = glm(Preg ~ Mass, data = D, family = "binomial")
summary(modelmass) 

#Make model predictions
predict(modelmass, newdata = data.frame(Mass = c(200, 300)), type = "response") #arbitrary
predict(modelmass,newdata=data.frame(Mass=c(219-(0.25*sd(D$Mass)),219+(0.25*sd(D$Mass)))),type="response") #half sd
...
D3 = D3 %>% select(Mass, rmean, se.rmean.) %>% mutate(Response = "LRS") #subsetting to columns used
D3$preg_mean = predict(modelmass, newdata = nd2)
D3$preg_SE = predict(modelmass, newdata = nd2, se.fit = T)$se.fit
D3$LRS = (D3$rmean - 2) * plogis(D3$preg_mean)

nd = 100000
for (i in 1:nrow(D3)) {
  LRS_draws = (rnorm(nd, D3$rmean[i], D3$se.rmean.[i]) - 2) *
    plogis(rnorm(nd, D3$preg_mean[i], D3$preg_SE))
  D3$LRS_low[i] = quantile(LRS_draws, probs = c(0.025))
  D3$LRS_high[i] = quantile(LRS_draws, probs = c(0.975))
}

D4 = data.frame(
  Y_Value = c(D3$rmean - 2, D3$LRS),
...
D$Size = 1.4
D$label = NA
#set default aesthetics for most points

allcols = c(viridis(10)[c(1, 4, 9)]) #colors for repeated measures seals
for (i in 1:length(seals)) {
  #loop through duplicated seals to set different shapes, colors, etc
  ...
  D$label[D$FieldID == seals[i] &
            D$Preg == 0] = as.character(seals[i]) #label for repeated measures seal
}


#prediction for manuscript
massV = seq(50, 450, by = 1)
yrv = 8 #setup inputs for prediction
nd1 = data.frame(  #dataframe for prediction
  Dist = mean(D2$Dist),
  Mass = rep(massV, length(yrv)),
  Date = mean(D2$Date),
  Depth = mean(D2$Depth),
  AgeWhenTracked = rep(yrv, each = length(massV)),
  Lifespan = rep(yrv + 1, each = length(massV)),
  Dead = 1,
  Response = "Survival"
)

nd1$Y_Value = predict(c4,
                      newdata = nd1,
                      type = "survival",
                      se.fit = T)$fit #predict values
nd1 = nd1 %>% mutate(label = if_else(Mass == min(Mass), as.character(paste0(
  "Age ", AgeWhenTracked)), NA_character_)) #labels
subset(nd1, Mass == round(quantile(D$Mass, .1)))
subset(nd1, Mass == round(quantile(D$Mass, .9)))

Dtemp = D %>% select(Dist:Date, Depth, Preg, Size, Shape, Color, label) %>%
  mutate(Response = "Reproduction") #re-organize
...
#FIGURE 2 (annual survival and reproduction)----

y_lab = c("Annual Probability of Reproduction",
          "Annual Probability of Survival") #specify y labels
...
p3 = ggplot(D5, aes(x = Mass, y = Y_Value)) + #make composite plot of survival and reproduction
  geom_smooth(
    data = subset(D5, Response == "Reproduction"),
    aes(x = Mass, y = Y_Value),
    method = glm,
  ...
  geom_label_repel(
    data = D5[which(D5$Shape > 21 &
                      D5$Y_Value == 0), ],
    aes(label = label),
    force = 0,
    nudge_y = .12,
    col = D5[which(D5$Shape > 21 &
                     D5$Y_Value == 0), "Color"],
    fontface = "bold"
  ) +
  geom_point(
    data = D5[D5$Response == "Reproduction", ],
    size = D5[D5$Response == "Reproduction", "Size"],
    bg = D5[D5$Response == "Reproduction", "Color"],
    stroke = 1,
  ...
  geom_line(data = D5[D5$Response == "Survival", ], size = 2) +
  ylim(0, 1) +
  labs(x = "", y = "") + theme(strip.placement = "outside") +
  theme(
...
#new function to wrap text annotation within the figure
